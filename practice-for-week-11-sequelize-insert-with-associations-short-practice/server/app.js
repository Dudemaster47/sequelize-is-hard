...
app.post("/bands/:bandId/musicians", async (req, res, next) => {
  ...
  const addMusician = await band.createMusician({
    firstName: firstName,
    lastName: lastName,
  });
  res.json({
...
app.post("/musicians/:musicianId/instruments", async (req, res, next) => {
    ...
    let instrument = instrumentIds
    let addInstrument = await musician.addInstruments(instrument);
    res.json({
      message: "Associated musician with instruments",
